(() => {
  ...
  function solve(state) {
    ...
    for (const domino of dominoes) {
      ...
      for (const node of nodes) {
        for (const direction of ["right", "down"]) {
          const otherId = node[direction];
          if (!otherId || !nodeById.has(otherId)) continue;

          for (const orientation of orientations(domino)) {
            const key = `${node.id}/${otherId}/${orientation.top}/${orientation.bottom}`;
    ...
    const ordered = dominoes
      .map((domino, index) => ({ domino, placements: candidatePlacements[index] }))
      .sort((a, b) => a.placements.length - b.placements.length);

    const result = search(ordered, constraints, new Map(), [], new Set(), nodes.length);
  ...
  function search(orderedDominoes, constraints, valuesByNode, placements, usedNodes, targetNodeCount) {
    if (placements.length === orderedDominoes.length) {
      return usedNodes.size === targetNodeCount && constraintsSatisfied(constraints, valuesByNode)
        ? { placements, valuesByNode: Object.fromEntries(valuesByNode) }
        : null;
    }

    const { placements: candidates } = orderedDominoes[placements.length];
    for (const placement of candidates) {
      if (usedNodes.has(placement.topNode) || usedNodes.has(placement.bottomNode)) continue;
      ...
      if (constraintsStillPossible(constraints, valuesByNode)) {
        const result = search(
          orderedDominoes,
          constraints,
          valuesByNode,
  ...
  function constraintsStillPossible(constraints, valuesByNode) {
    return constraints.every(({ nodes, constraint }) => {
      ...
      if (constraint?.type === "sum") {
        ...
        if (constraint.sign === "<") return assigned.length === values.length ? sum < target : sum < target;
